...
def run(im, reference):
    ...
    shifted_back = rotate(im, transform.rotation, clip=True)
    aligned = warp(shifted_back, AffineTransform(translation=transform.translation), mode="constant", preserve_range=False)

    show_ims(reference, im, title="originals")
    show_ims(aligned, aligned, title="comparison")

    print(f"Aligned in {time.time() - start}s")
    return transform, get_relative_translation(im, *transform.translation)
...
def get_relative_translation(im: np.ndarray, x, y):
    """
    Computes relative/percentual shift given the absolute shift for an image.
    :param im: Unshifted image.
    :param x: Absolute shift in x direction.
    :param y: Absolute shift in y direction.
    :return: Relative shift in y and x direction respectively.
    """
    rows, cols, _ = im.shape
    print(f'relative y: {y/rows}, relative x: {x/cols}')
    return y/rows, x/cols



def compute_transform(im, reference) -> AffineTransform:
    """
    Computes the affine transformation to align a given image to the given reference.
    :param im: Image to be aligned.
    :param reference: Reference to align the given image to.
    :return: Affine matrix representing the transformation.
    """
    base, rt = crop_to_same_size(reference, im)
    base = rgb2gray(rgba2rgb(base)) if base.shape[2]==4 else rgb2gray(base)
    gray_rt = rgb2gray(rgba2rgb(rt)) if rt.shape[2]==4 else rgb2gray(rt)
    ...
    transform = AffineTransform(rotation=-recovered_angle, translation=(shift_x, shift_y), scale=shift_scale)
